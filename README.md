...
| Safe Runner | Dry-run first, execute only low-risk commands when explicitly requested, and save run records. | 默认 dry-run，仅在明确选择时执行低风险命令，并保存运行记录。 |
...
The demo shows paper-grounded result extraction, code-aware command planning, safe execution, log-based metric parsing, evidence-aware verification, and reproduction report generation.

该 demo 展示论文证据抽取、代码感知命令规划、安全执行、日志指标解析、证据核验和复现实验报告生成。
...
5. Run in dry-run mode, or execute safe commands only when requested.
...
3. 分析关联代码仓库。
4. 规划候选复现命令。
5. 使用 dry-run，或在明确选择时执行 safe 命令。
6. 解析日志并抽取指标。
7. 对比复现指标和论文报告指标。
8. 核验证据来源和缺失证据。
9. 生成结构化 Markdown 报告。
...
Use **dry-run only** first to inspect paper metrics, repository entry points, candidate commands, and evidence checks. Select **run safe commands** only for low-risk command execution.

优先使用 **dry-run only** 检查论文指标、仓库入口、候选命令和证据核验。仅在需要时选择 **run safe commands** 执行低风险命令。
...
- 默认复现模式是 dry-run。
- Safe commands run only when explicitly requested.
- 只有明确选择时才执行 safe 命令。
